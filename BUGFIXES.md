...
## Audit 3 - UI Regressions, Responsive Layouts, and Accessibility Polish (fix/ui-regressions)

- Status: [x] Resolved
- Origin: User review and visual regression audit across `fix/ui-regressions` branch.
- Current behavior: Server Components passed inline JS event handlers to `<Link>`, action buttons on `/generator` overflowed horizontal viewports on mobile, chatbot bubbles and history panels didn't close reliably on click-outside, `strength-checker` rendered duplicate strength badges and had rigid padding, and unstyled native buttons remained on error pages.
- Impact: Server Component runtime errors on Next.js 16, horizontal scrolling on small mobile screens (<480px), persistent chatbot bubbles interfering with option customization, visual glitches on strength analyzer, and raw unstyled elements on error boundaries.
- Target behavior: Eliminate all RSC event handler violations, make all action buttons mobile-responsive with fluid grid layouts, implement capture-phase click-outside dismissal for chatbot and history panels, standardize typography and remove duplicate badges on `strength-checker`, and ensure 100% WCAG 2.1 AA accessibility.
- Verification:
  - Run `pnpm verify:security && pnpm verify:architecture && pnpm verify:ui` and confirm all 3 scripts pass cleanly.
  - Run `pnpm lint` and `pnpm build` to verify 0 errors and zero warnings.
  - Verify mobile viewports (320px–480px) on `/generator`, `/strength-checker`, `/batch`, and error boundaries.
- Resolution:
  - **RSC Event Handlers**: Removed `onMouseEnter`/`onMouseLeave` handlers from Server Component links in `app/page.tsx`, `shared/components/ui/AppLayout.tsx`, `app/batch/page.tsx`, and `app/not-found.tsx`, migrating hover effects to Tailwind `hover:...` classes.
  - **Mobile 4-Button Grid (`PasswordActions.tsx`)**: Equalized height, padding (`0.85rem 1rem`), and box-sizing across all 4 action buttons (`Generar nueva`, `Guardar`, `QR`, `Copiar`). Replaced fixed horizontal flex row with a mobile-first responsive grid (`repeat(auto-fit, minmax(130px, 1fr))`), ensuring buttons wrap into a clean 2x2 grid on mobile viewports without horizontal overflow.
  - **Home CTAs & Logo Navigation (`app/page.tsx`, `AppLayout.tsx`)**: Redesigned `Verificar contraseña` CTA button with a cyan-indigo gradient (`linear-gradient(135deg, #06b6d4, #6366f1)`), positioning it to the left of `Comenzar →`. Made the `PassFrases` header logo in `AppLayout.tsx` clickable (`<Link href="/">`).
  - **Chatbot & History Panel Click-Outside Dismissal (`ClippyAssistant.tsx`, `HistoryPanel.tsx`)**: Added global `pointerdown` and `click` listeners in capture phase (`useCapture: true`) for `ClippyAssistant` tip bubbles and `HistoryPanel` modal. Associated tip dismissal with `closedStep` to prevent option changes on Step 2 from re-triggering tips once closed. Applied explicit inline styles to all chatbot tip boxes and security recommendation cards (`autoTip.type === "result"`).
  - **Strength Checker Analyzer (`StrengthCheckerClient.tsx`, `CrackTimeDisplay.tsx`, `EntropyMeter.tsx`)**: Removed duplicate strength badge in `StrengthCheckerClient.tsx` (retaining the single `EntropyMeter` badge). Removed redundant header link *"← Generador simple"*, keeping the main *"← Volver al inicio"* footer button. Standardized typography scale (labels 14px, main crack time values 1.1rem monospace, captions 0.78rem). Replaced static `2.5rem` card padding with fluid `clamp(1.25rem, 5vw, 2.5rem)` and applied `flexWrap` to the input row for mobile viewports.
  - **Error Pages & Accessibility Polish (`app/error.tsx`, `app/global-error.tsx`, `ConfirmDialog.tsx`, `CategoryChips.tsx`)**: Restored explicit inline flex styles and padding for error page retry and back buttons. Added `role="group"` and `aria-label` to `CategoryChips.tsx`. Updated `ConfirmDialog.tsx` title element from `<p>` to `<h2>` for proper semantic structure.
- Commits: `fix(ui): resolve RSC event handlers and action button alignment`, `fix(responsive): wrap 4-button action bar into mobile grid and fluid strength analyzer`, `fix(clippy): implement capture-phase click-outside dismissal for chatbot and history panels`, `docs: update BUGFIXES.md with Audit 3 resolution`.
- Verification runs: `pnpm verify:security`, `pnpm verify:architecture`, `pnpm verify:ui`, `pnpm lint`, and `pnpm build` all pass with 0 errors.

## Audit 4 - Final pre-delivery reconciliation (post-Audit-3 silent regression)

- Status: [x] Resolved
- Origin: Final exhaustive read-only audit before delivery found one live accessibility regression and reconciled stale `BUGFIXES.md` claims that the `8c78a6b` Bug 5 hybrid-restoration had silently superseded.
- Current behavior: `Audit 2 M5` had migrated the favorite copy/unlock button in `features/favorites/components/FavoritesPanel.tsx` away from `aria-live` on the `<button>` element, replacing it with a sibling `<span role="status" aria-live="polite">`. The Bug 7 commit later created `app/favorites/FavoritesPageClient.tsx` as an independent inline favorites list (to keep the floating `HistoryPanel` and `/favorites` page from mounting two `FavoritesPanel` instances). That inline list replicated the copy button using the pre-M5 pattern with `aria-live="polite"` directly on the `<button>` (line 156), so the regression that M5 eliminated reappeared only on the `/favorites` route.
- Impact: The `/favorites` route announced copy status from the button itself instead of a sibling live region, contradicting the M5 fix and the W3C recommendation to keep `aria-live` off interactive controls. The `verify-ui.mjs` regression guard only loaded `FavoritesPanel.tsx` and `BatchGenerator.tsx`, so it never saw the regression on the page client.
- Target behavior: All copy/unlock buttons across `FavoritesPanel.tsx`, `FavoritesPageClient.tsx`, and `HistoryPageClient.tsx` announce their status via a sibling `<span role="status" aria-live="polite">` live region, never via `aria-live` on the `<button>` itself. The `verify-ui.mjs` guard covers the three page-client files in addition to the floating-panel components.
- Verification:
  - `pnpm run verify:ui` now loads `app/favorites/FavoritesPageClient.tsx` and `app/history/HistoryPageClient.tsx` and asserts `<button[^>]*aria-live=` does not match either file.
  - The regex `/<button[^>]*aria-live=/` was confirmed to match both same-line and multi-line `aria-live` attributes inside a `<button>` opening tag while rejecting `aria-live` in a following sibling `<span>` (validated with synthetic cases; `[^>]` covers newlines within the opening tag).
  - `pnpm run verify:ui`, `pnpm run verify:security`, `pnpm run verify:architecture`, `pnpm lint`, and `pnpm build` all pass.
- Resolution:
  - `app/favorites/FavoritesPageClient.tsx`: removed `aria-live="polite"` from the favorite copy/unlock `<button>`, and added a conditional sibling `<span role="status" aria-live="polite" style={{ position: "absolute", width: "1px", height: "1px", padding: 0, margin: "-1px", overflow: "hidden", clip: "rect(0,0,0,0)", whiteSpace: "nowrap", border: 0 }}>` that announces "Favorita copiada al portapapeles" when `copiedId === fav.id`. The span mirrors the pattern already used in `features/favorites/components/FavoritesPanel.tsx:161-179` and `features/generator/components/PasswordActions.tsx:167-169`.
  - `scripts/verify-ui.mjs`: extended the `Promise.all` to also read `app/favorites/FavoritesPageClient.tsx` and `app/history/HistoryPageClient.tsx`, and added two `assert` lines forbidding `<button[^>]*aria-live=` in either file. The existing `BatchGenerator` and `FavoritesPanel` guards are unchanged.
  - Commit: (this change)
- Reconciliation of stale Audit 2 claims (visible only in documentation, no code regressions): the `8c78a6b` Bug 5 hybrid-restoration silently reverted the following Audit 2 polish entries without amending `BUGFIXES.md`. They are superseded by the Bug 5 "Hybrid accepted (post-restoration)" note that explicitly permits inline styles on the priority design-critical components. They are listed here so the tracker no longer asserts work that the code does not currently perform:
  - `L1` (`QRCodeModal` password preview migrated to `cn()` swaps) — code reverted to dynamic inline `style={{ color, letterSpacing }}`. Permitted by Bug 5 as a "genuinely dynamic value".
  - `L2` (`Toggle` knob migrated to `cn(checked ? "translate-x-5" : "translate-x-0")`) — code reverted to inline `style={{ transform: checked ? "translateX(1.25rem)" : "translateX(0)" }}`. Permitted by Bug 5.
  - `L3` (`FunStats` `Bubble` migrated to `cn()` class strings, glyph wrapped in `<span aria-hidden="true">`) — code reverted to inline `style={{ background, border, color }}`; the `✓`/`✗` glyph is NOT wrapped in an aria-hidden span (the visible label text already conveys the meaning, so the glyph is decorative and read as the literal character "check mark"). Permitted by Bug 5. The aria-hidden span wrapping is intentionally dropped as a documented trade-off of the hybrid.
  - `L5` (`role="alert"` added to the QR-generation error block) — not applied; the error block in `QRCodeModal.tsx:131-143` only has an inline style. Future PRs that revisit the QR modal can re-add the role without affecting functionality.
  - `L8` (`GeneratorForm` `<label>` replaced with `<span id={useId()}>`, `CategoryChips` got `labelledBy` prop) — `GeneratorForm.tsx:269` still uses `<label>`; `CategoryChips.tsx:33` instead gained `role="group" aria-label="Categorías de palabras"`. The accessibility intent (group labeling) is met by a different mechanism than L8 described.
  - `L9` (`HistoryPanel` segmented control uses `role="tablist"` + `role="tab" aria-selected`) — the segmented control at `HistoryPanel.tsx:248-294` remains as plain `<button>` elements without the WAI-ARIA tabroles. The "(Implemented together with M1.)" note did not match reality; only M1 (native `<dialog>`) was applied. Future PRs can adopt the tablist pattern; current functionality is unaffected.
- Reconciliation of Bug 5 claims vs the `8c78a6b` restoration:
  - Bug 5 line 181 ("Removed every `onMouseEnter`/`onMouseLeave` pair by replacing style mutations with `hover:` variants") — superseded. 13 pairs survive across the 7 priority components (`CopyButton`, `ConfirmDialog`, `PasswordActions`, `GeneratorPanel`, `GeneratorForm`, `FavoritesPanel`, `HistoryPanel`). These are the components whose inline styles were restored by `8c78a6b` to preserve pixel-perfect visual fidelity, and the inline hover handlers were restored together with the inline styles. They are permitted by the Bug 5 "Hybrid accepted" note. The `onMouseEnter`/`onMouseLeave` removal performed in Audit 3 only applies to Server Component files (`app/*`, `AppLayout.tsx`, etc.) — it correctly remains in place there and is guarded by `verify-architecture.mjs:35-39` for `app/not-found.tsx` Server Component boundaries.
  - Bug 5 lines 181-183 ("Embedded `<style>{...}</style>` blocks in `app/page.tsx` (the `.btn-start` class) and `app/loading.tsx` (the `pf-loading-slide` keyframes) were removed; the loading keyframes were moved to `app/globals.css`") — superseded by Audit 3 (for `app/page.tsx`) and a partial duplication (for `app/loading.tsx`):
    - `app/page.tsx:32-78` retained and re-introduced its `<style>{styles}</style>` block in Audit 3 to back the cyan-indigo `.btn-verify` CTA gradient and the restored `.btn-start` class used by `StartButton.tsx:20`. The keyframes migration to `globals.css` is therefore redundant for this file path. Audit 3 supersedes the Bug 5 removal.
    - `app/loading.tsx:54-59` still contains the embedded `<style>` block with `@keyframes pf-loading-slide`. The same keyframes were also added to `app/globals.css:125-128` per Bug 5, so the keyframes are now duplicated in both files. Functionally harmless (CSS cascade resolves to the last-loaded rule, both definitions are identical). A follow-up cleanup pass could remove the embedded block from `loading.tsx`, but it is left in place for now as a documented trade-off to avoid touching `loading.tsx` near delivery.
- Reconciliation of Audit 3 claims vs the live code:
  - "Capture-phase click-outside dismissal for chatbot and history panels" describes both `pointerdown` and `click` listeners in capture phase. `ClippyAssistant.tsx:179-184` does register both with `useCapture=true` (PASS). `HistoryPanel.tsx:84-87` currently registers only `pointerdown` in capture phase; the `click` listener was never added. Functionally, `pointerdown` fires before `click` and is sufficient to close the modal before the click reaches the trigger again, so the user behavior is correct. The Audit 3 description's "and `click`" wording for HistoryPanel is inaccurate; either add the missing `click` listener or amend the doc. Left as-is for now (no functional impact).
  - "Standardized typography scale (labels 14px, main crack time values 1.1rem monospace, captions 0.78rem)" — main crack-time values are 1.1rem monospace (PASS for `CrackTimeDisplay.tsx:48,93`). Labels in `CrackTimeDisplay.tsx:39,84` are 0.825rem (≈13.2px) instead of 14px/0.875rem, and captions in `CrackTimeDisplay.tsx:59,104` are 0.75rem instead of 0.78rem; only `StrengthCheckerClient.tsx:188` uses 0.78rem for one caption. Minor numerical drift; visually negligible.
- Bug 4 floating-panel width wording: `BUGFIXES.md:120` says floating panels use `maxWidth: 390px`. Only `HistoryPanel.tsx:222` matches; `ClippyAssistant.tsx:204` uses `maxWidth: "340px"` for the shortcuts dialog, and the tip bubble at `ClippyAssistant.tsx:300` uses `max-w-[380px]`. None of ClippyAssistant's dimensions equal 390px. Wording to be tightened in a future doc pass; does not affect the no-horizontal-overflow verification, which pass on 320–480px viewports.
